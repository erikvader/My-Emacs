...
#+begin_src emacs-lisp :tangle yes
  ...
  returns t if successful, nil otherwise"
    (interactive "p\ncJump to char: ")
    (setq eriks-repeat-last-command (cons 'eriks-find-char (list ARG c)))
    (let ((dir (if (>= ARG 0)
                   1
                 -1))
          (start (point))
  ...
  (defun eriks-find-char-backward (ARG c)
    ...
    (setq eriks-repeat-last-command (cons 'eriks-find-char-backward (list ARG c))))

  (defun eriks-repeat ()
    "If the last command is an erik-function, then repeat it using the infromation in the variable `eriks-repeat-last-command'
  Else do a normal `repeat'"
    ...
    (if (and
         (boundp 'eriks-repeat-last-command)
         (eq last-repeatable-command (car eriks-repeat-last-command)))
        (apply (car eriks-repeat-last-command) (cdr eriks-repeat-last-command))
    ...
    ;;       ((or
    ;;         (eq last-repeatable-command 'eriks-zap-to-char)
    ;;         (eq last-repeatable-command 'eriks-zap-to-char-backwards)
    ;;         (eq last-repeatable-command 'eriks-zap-up-to-char)
    ;;         (eq last-repeatable-command 'eriks-zap-up-to-char-backwards))
    ;;        (eriks-zap-char-repeat))
    ;;       (t (call-interactively 'repeat)))
    )

  (defun eriks-insert-char (ARG c)
    (interactive "p\ncInsert: ")
    (setq eriks-repeat-last-command (cons 'eriks-insert-char (list ARG c)))
    (dotimes (i ARG)
      (insert c)))
...
#+end_src
** zap
kills text up to a given char c. If c is not found then nothing happens.
zap up to: not including c
zap to   :     including c
Stuff for repeating is saved in the variable 'eriks-zap-last-command'
#+begin_src emacs-lisp :tangle yes
  (defun eriks-zap-up-to-char (c)
    "Documentation somewhere inside myinit.org"
    (interactive "cZap up to: ")
    (setq eriks-zap-last-command `(eriks-zap-up-to-char ,c))
    (let ((start (point)))
      (if (eriks-find-char 1 c)
          (kill-region start (point)))))

  (defun eriks-zap-to-char (c)
    "Documentation somewhere inside myinit.org"
    (interactive "cZap to: ")
    (setq eriks-zap-last-command `(eriks-zap-to-char ,c))
    (let ((start (point)))
      (if (eriks-find-char 1 c)
          (kill-region start (+ 1 (point))))))

  (defun eriks-zap-up-to-char-backwards (c)
    "Documentation somewhere inside myinit.org"
    (interactive "cZap up to backwards: ")
    (setq eriks-zap-last-command `(eriks-zap-up-to-char-backwards ,c))
    (let ((start (point)))
      (if (eriks-find-char -1 c)
          (progn
            (forward-char)
  ...
  (defun eriks-zap-to-char-backwards (c)
    "Documentation somewhere inside myinit.org"
    (interactive "cZap to backwards: ")
    (setq eriks-zap-last-command `(eriks-zap-to-char-backwards ,c))
    (let ((start (point)))
      (if (eriks-find-char -1 c)
          (kill-region start (point)))))

  (defun eriks-zap-char-repeat ()
    "Documentation somewhere inside myinit.org"
    (interactive)
    (if (boundp 'eriks-zap-last-command)
        (funcall (car eriks-zap-last-command) (nth 1 eriks-zap-last-command))))
...
från stackoverflow
#+begin_src emacs-lisp :tangle yes
  (defun eriks-kill-thing-at-point (thing)
  ...
  (defun eriks-kill-word-at-point ()
    "Kill the word at point."
    (interactive)
    (eriks-kill-thing-at-point 'word))

  (defun eriks-kill-symbol-at-point ()
    "Kill the symbol at point."
    (interactive)
    (eriks-kill-thing-at-point 'symbol))
#+end_src
** swiper
#+begin_src emacs-lisp :tangle yes
  ...
  (defun swiper-thing-at-point (thing)
    (let ((bounds (bounds-of-thing-at-point thing)))
      (if bounds
          (swiper-selection (car bounds) (cdr bounds))
        (error "No %s at point" thing))))

  (defun swiper-selection (beg end)
    (interactive "r")
    (deactivate-mark)
...
#+begin_src emacs-lisp :tangle yes
  ...
  (modalka-define-kbd "M s" "C-M-SPC")
  (modalka-define-kbd "M b" "C-x h")
  (modalka-define-kbd "M w" "M-@")
  (modalka-define-kbd "M p" "M-h")
  (modalka-define-kbd "M d" "C-M-h")
  (define-key modalka-mode-map (kbd "M l") 'eriks-mark-line)
  ...
  (define-key modalka-mode-map (kbd "D e")   'kill-line)
  ;;(modalka-define-kbd               "D E"    "C-M-k")
  (define-key modalka-mode-map (kbd "D a")   'eriks-backward-kill-line)
  ...
  (define-key modalka-mode-map (kbd "D p")   'eriks-kill-paragraph)
  (define-key modalka-mode-map (kbd "D n")   'eriks-zap-to-char)
  (define-key modalka-mode-map (kbd "D N")   'eriks-zap-to-char-backwards)
  (define-key modalka-mode-map (kbd "D u")   'eriks-zap-up-to-char)
  (define-key modalka-mode-map (kbd "D U")   'eriks-zap-up-to-char-backwards)
  (define-key modalka-mode-map (kbd "D i w") 'eriks-kill-word-at-point)
  (define-key modalka-mode-map (kbd "D i W") 'eriks-kill-symbol-at-point)
  ...
  (modalka-kill-to-delete "p")
  (modalka-kill-to-delete "a")
  (modalka-kill-to-delete "n")
  (modalka-kill-to-delete "N")
  (modalka-kill-to-delete "u")
  (modalka-kill-to-delete "U")
  (modalka-kill-to-delete "i w")
  (modalka-kill-to-delete "i W")
  ...
  (define-key modalka-mode-map (kbd "S r") 'swiper-selection)
  (define-key modalka-mode-map (kbd "S w") 'swiper-word-at-point)
  (define-key modalka-mode-map (kbd "S W") 'swiper-symbol-at-point)

...
#+begin_src emacs-lisp :tangle yes
  ...
  (define-key swiper-map (kbd "<escape>") 'keyboard-escape-quit)
...
#+begin_src emacs-lisp :tangle yes
  ...
  (sp-local-pair '(c-mode java-mode css-mode) "{" nil :post-handlers '((my-create-newline-and-enter-sexp "RET")))
...
#+begin_src emacs-lisp :tangle yes
  ...
  ;; C-u makes it prompt for a face for some nice reason
  (evil-define-operator eriks-evil-highlight-operator (beg end type)
    ...
    (let* ((hi-lock-auto-select-face t)
           ...
           (reg (regexp-quote (buffer-substring beg end))))
      (hi-lock-face-buffer reg face)))

  (defun eriks-evil-highlight-regex ()
    "highlights a given regex written in minibuffer interactively.
  Uses a default face unless C-u is used."
    (interactive)
    (let* ((reg (read-regexp "highlight" "" 'evil-ex-search-history))
           (hi-lock-auto-select-face t)
           (face (hi-lock-read-face-name)))
      (hi-lock-face-buffer reg face)))

  (define-key evil-normal-state-map (kbd "SPC h h") 'eriks-evil-highlight-operator) ;; highlight motion,     auto face unless C-u
  (define-key evil-visual-state-map (kbd "SPC h h") 'eriks-evil-highlight-operator) ;; highlight selection,  auto face unless C-u
  ...
  ;; (define-key evil-visual-state-map (kbd "SPC h u") 'unhighlight-regexp)

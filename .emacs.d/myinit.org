...
#+begin_src emacs-lisp :tangle yes
  ...
  (define-key modalka-action-region-special-map (kbd "n") 'eriks-find-inclusive)
  ...
  (define-key modalka-action-region-special-map (kbd "U") 'eriks-find-inclusive-backward)

  (defun eriks-find-inclusive ()
    (interactive)
    (if (call-interactively 'eriks-find-char)
        (forward-char)))

  (defun eriks-find-inclusive-backward ()
    (interactive)
  ...
  (defun modalka-action-region (prompt)
    (if (region-active-p)
  ...
  (defun modalka-swiper ()
    ...
    (let ((region (modalka-action-region "Swiper: ")))
      (if region
          (swiper-selection (car region) (cdr region))
        (message "no region"))))

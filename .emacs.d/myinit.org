...
#+begin_src emacs-lisp :tangle yes
  ...
  (defun toggle-modalka (&optional cmd)
    "
  cmd == 0                           => turn off
  cmd == 1                           => turn on
  cmd == nil or cmd == undefined typ => toggle"
    (interactive)
    (modalka-mode (if (null cmd) 'toggle cmd))
    (if (bound-and-true-p modalka-mode)
        (setq modalka-insert-text nil)))

  (define-key my-keys-map (kbd "§") 'toggle-modalka)

  (define-key modalka-mode-map (kbd "t c") 'transpose-chars)
  (define-key modalka-mode-map (kbd "t w") 'transpose-words)
  (define-key modalka-mode-map (kbd "t l") 'transpose-lines)
  (define-key modalka-mode-map (kbd "t r") 'eriks-region-switch)
  (define-key modalka-mode-map (kbd "t R") 'eriks-region-switch-abort)

  (define-key modalka-mode-map (kbd "A") 'beginning-of-defun)
  (define-key modalka-mode-map (kbd "E") 'end-of-defun)

  ;;(define-key modalka-mode-map (kbd "W") 'eriks-copy-dwim)
  ...
  (define-key modalka-mode-map (kbd "p s") 'sp-splice-sexp)
  (define-key modalka-mode-map (kbd "p S") 'sp-split-sexp)
  (define-key modalka-mode-map (kbd "p J") 'sp-join-sexp)
  (define-key modalka-mode-map (kbd "p f b") 'sp-forward-barf-sexp)
  (define-key modalka-mode-map (kbd "p f s") 'sp-forward-slurp-sexp)
  (define-key modalka-mode-map (kbd "p b b") 'sp-backward-barf-sexp)
  (define-key modalka-mode-map (kbd "p b s") 'sp-backward-slurp-sexp)
  (define-key modalka-mode-map (kbd "p k") 'sp-kill-sexp)
  (define-key modalka-mode-map (kbd "p h k") 'sp-kill-hybrid-sexp)
  (define-key modalka-mode-map (kbd "p h b") 'sp-dedent-adjust-sexp)
  (define-key modalka-mode-map (kbd "p h s") 'sp-slurp-hybrid-sexp)
  ...
  ;;avy
  (define-key modalka-mode-map (kbd "G") 'avy-goto-char-timer) ;;avy-goto-char
  ...
  (define-key modalka-mode-map (kbd "C") 'modalka-mc)

  (define-key modalka-mode-map (kbd "M-+") 'evil-numbers/inc-at-pt)
  (define-key modalka-mode-map (kbd "M--") 'evil-numbers/dec-at-pt)
#+end_src
** action region
#+begin_src emacs-lisp :tangle yes
  ...
  (defun modalka-kill-delete-region (name func)
    (let ((region (swap-cons (modalka-action-region name))))
      (if region
          (progn
            (funcall func (car region) (cdr region))
            ;; (message "%d : %d" (car region) (cdr region))
            )
        (message "no region"))))

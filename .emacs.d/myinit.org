...
#+begin_src emacs-lisp :tangle yes
  ...
  (defun eriks-find-char-alt-backward ()
   ...
   ;;keys som det senaste kommandor hade
  (setq eriks-repeat-cur-macro nil)
  ...
  (setq prev-command-keys nil)
   ...
   ;;key-sequence som senast ändrade på buffern. Denna som
   ;;`eriks-repeat' kommer att köra
  (setq eriks-repeat-cur-change-macro nil)

  ;;interna grejer :)
  (setq eriks-repeat-before-change-ran nil)

  (defun eriks-repeat-post-command ()
    (if (not (or
              (eq real-this-command 'eriks-repeat)
              (eq real-this-command 'undo-tree-undo)))
        (progn
          (setq eriks-repeat-cur-macro (vconcat prev-command-keys (this-command-keys-vector)))
          (setq prev-command-keys nil)
          (if eriks-repeat-before-change-ran
              (progn
                (setq eriks-repeat-cur-change-macro eriks-repeat-cur-macro)
                (setq eriks-repeat-before-change-ran nil))))))

  (defun eriks-repeat-before-change (a b c)
    (setq eriks-repeat-before-change-ran t))

  (setq inhibit-modification-hooks nil)
  (add-hook 'post-command-hook 'eriks-repeat-post-command)
  (add-hook 'before-change-functions 'eriks-repeat-before-change)

  (defun eriks-repeat ()
    "Repeatar senaste key-sequencen som gjorde en ändring i buffern."
    (interactive)
    ...
    (message "repeating %s" eriks-repeat-cur-change-macro)
    (execute-kbd-macro eriks-repeat-cur-change-macro))
...
#+begin_src emacs-lisp :tangle yes
  ...
  ;;TODO prefix arg!! typ C-e gillar inte att den får 0

...
#+begin_src emacs-lisp :tangle yes
  ...
  returns t if successful, nil otherwise"
    (interactive "p\ncJump to char: ")
    (eriks-repeat-prepare-last 'eriks-find-char (list ARG c))
    (let ((dir (if (>= ARG 0)
                   1
                 -1))
          (start (point))
  ...
  (defun eriks-find-char-backward (ARG c)
    ...
    (eriks-repeat-prepare-last 'eriks-find-char-backward (list ARG c)))
  ...
  (defun eriks-repeat-prepare-last (func arglist)
    "Prepare 'func' to be repeated with args 'arglist'.
  If the `last-repeatable-command' is 'func', then run 'func' with 'arglist'"
    (setq eriks-repeat-last-command (cons func arglist)))

  (defun eriks-repeat-set-last (func arglist)
    "sets `eriks-repeat-prepare-last' and `last-repeatable-command'"
    (eriks-repeat-prepare-last func arglist)
    (setq last-repeatable-command func))
  ...
  Else do a normal `repeat'"
    ...
    ;; (message "last: %s, eriks-last: %s" last-repeatable-command (car eriks-repeat-last-command))
    (if (and
    ...
    ;;         (eq -repeatable-command 'eriks-zap-to-char-backwards)
...
#+begin_src emacs-lisp :tangle yes
  ...
  (setq modalka-action-region-last-keys nil)
  (defun modalka-action-region (prompt &optional KEYS)
    (if (region-active-p)
      ...
      (unwind-protect
          (let ((keys KEYS)
            ...
            (setq modalka-action-region-last-keys keys)
            (setq return (call-interactively (key-binding keys)))
            (message "")
            (if (is-bounds return)
                return
              (progn
  ...
  (defun modalka-select ()
    (interactive)
    (let ((region (modalka-action-region "Select: ")))
      (if region
          (progn
            (goto-char (car region))
            (set-mark-command nil)
            (goto-char (cdr region))
            (setq real-this-command (key-binding modalka-action-region-last-keys))) ;;funkar inte med u U
        (message "no region"))))
  ...
  ;;(define-key modalka-mode-map (kbd "D i p")   'eriks-kill-paragraph)
...
(defun eriks-repeat-fetch (curc)
  (let ((list nil)
        (cur nil)
        (keys (recent-keys)))
    (catch 'break
      (dotimes (i 300)
        (setq cur (aref keys (- 299 i)))
        (setq list (cons cur list))
        (if (equal cur curc)
            (throw 'break))))
    (vconcat list)))
...
(defun test ()
  (interactive)
  (message "%s" (eriks-repeat-fetch (kbd "C-S-o C-x"))))

(global-set-key (kbd "C-S-o") 'test)
